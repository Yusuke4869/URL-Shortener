...
md
ファイルのソースとレンダリング後の両方で読みやすいドキュメントを作成することを重視します。
...
```markdown
このプロジェクトは TypeScript で開発されています。
```

**数値記号の使用**: 数値や記号は半角英数字を使用します。

```markdown
モジュールは 5 つに分かれています。 インデントは 2 スペースです。 バージョン
2.1.0 をリリースしました。
```
...
実装方法については[開発手順](./development-guide.md#実装手順)の該当セクションを確認してください。
```
...
```markdown
詳細は[Node.js 公式ドキュメント](https://nodejs.org/docs)を参照してください。

[React ライブラリ](https://react.dev/)の使用方法については公式サイトで確認できます。
```
